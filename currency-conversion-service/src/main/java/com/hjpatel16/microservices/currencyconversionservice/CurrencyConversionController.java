package com.hjpatel16.microservices.currencyconversionservice;
...
public class CurrencyConversionController {
	...
	private final String CURRENCY_EXCHANGE_URI = "http://localhost:8000/currency-exchange/from/{from}/to/{to}";
	...
	public CurrencyConversion convertCurrency(@PathVariable String from, @PathVariable String to, @PathVariable BigDecimal quantity) {
		...
		//Uses a rest template to invoke currency exchange service
		ResponseEntity<CurrencyConversion> responseEntity = new RestTemplate().getForEntity(CURRENCY_EXCHANGE_URI, CurrencyConversion.class, uriVariables);
	...
	public CurrencyConversion convertCurrencyFeign(@PathVariable String from, @PathVariable String to, @PathVariable BigDecimal quantity) {
		...
		//Invoking currency exchange service through feign
